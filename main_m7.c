...
/* microseconds per unit of edge weight */
#define USEC_PER_WEIGHT  5000000
#define NODE_WAIT_SECONDS 1
#define OUTSIDE_NODE     45.0f
/* how long to hold the semaphore after departure for visual alignment */
...
typedef struct {
    ...
    int   current_node;
    int   next_node;
    int   remaining;   /* hops left in path — used by SJF */
} IPCMessage;

/* ------------------------------------------------------------------ */
/* Per-node wait queue entry                                           */
/* ------------------------------------------------------------------ */
typedef struct {
    int traveler_index;
...
/* ------------------------------------------------------------------ */
/* Traveler state (parent side)                                        */
/* ------------------------------------------------------------------ */
typedef struct {
    int   src, dst;
    pid_t pid;
    int   pipe_fd[2];  /* child writes, parent reads */
    int   ack_fd[2];   /* parent writes, child reads  */
...
static void child_process(Graph *g, Traveler t, int index,
    ...
    for (int i = 0; i < path_len; i++) {
        ...
        int remaining = path_len - 1 - i;   /* hops left after this node */

        /* --- Tell parent we want to enter the node --- */
        send_message(write_fd, MSG_WAITING, index, current, next, remaining);

        /* --- Block until parent grants entry (ACK) --- */
        char ack;
        read(ack_read_fd, &ack, 1);

        /* --- We are now inside the node --- */
        send_message(write_fd, MSG_NODE, index, current, next, remaining);
        sleep(NODE_WAIT_SECONDS);

        if (next != -1) {
            /* Signal we are about to leave so the GUI starts moving us */
            send_message(write_fd, MSG_TRAVELING, index, current, next, remaining);
            /*
             * Hold the node for a short extra moment so the traveler
             * visually clears it before we tell the parent to admit
             * the next waiter.
             */
            usleep(VISUAL_LEAVE_DELAY);
        }

        /* --- Tell parent the node is free (release token) --- */
        /* We signal "node free" by sending a special message type 99 */
        {
            IPCMessage rel_msg;
            rel_msg.type           = 99;
            rel_msg.pid            = getpid();
...
/* Remove entry at position k (shift left) */
...
/*
 * Pick the next traveler from the queue according to the chosen
 * scheduling algorithm and return its index in the queue.
...
static void handle_message(IPCMessage msg, Traveler travelers[],
    ...
    /* ---- Child is travelling to next node ---- */
    case MSG_TRAVELING: {
        travelers[i].waiting      = 0;
        travelers[i].waiting_node = -1;
        travelers[i].current_node = msg.current_node;
    ...
    case MSG_FINISHED:
        ...
        /* Important: do NOT free the node here.
         * The child already sends type 99 after leaving every node,
         * including the destination node. Releasing again here can
         * admit two travelers to the same node by mistake.
         */
        break;
...
int main(int argc, char *argv[]) {
    ...
    /* ---- Parse command line: ./sim -schd fcfs|sjf <file> ---- */
    ...
    fprintf(stderr, "Usage: %s -schd fcfs|sjf|priority <graph_file>\n", argv[0]);
    return 1;
}

    filename = argv[3];

    /* ---- Load graph and travelers ---- */
    Graph    *g              = NULL;
    ...
    for (int i = 0; i < traveler_count; i++) {
        ...
        if (pid < 0) {
            perror("fork");
            close(travelers[i].pipe_fd[0]); close(travelers[i].pipe_fd[1]);
            close(travelers[i].ack_fd[0]);  close(travelers[i].ack_fd[1]);
        } else if (pid == 0) {
            ...
            /* Make data pipe non-blocking for the GUI loop */
    ...
    while (!WindowShouldClose()) {
        ...
        for (int i = 0; i < traveler_count; i++) {
            ...
            IPCMessage msg;
            while (read(travelers[i].pipe_fd[0], &msg, sizeof(msg)) ==
                   sizeof(msg)) {
            ...
            if (travelers[i].waiting) {
                ...
                for (int j = 0; j < i; j++) {
                    if (travelers[j].waiting &&
                        travelers[j].waiting_node ==
                        travelers[i].waiting_node)
                        rank++;
                }
                offset_y = rank * 26;
            }
        ...
        /* Show active scheduler in header */
        char subtitle[64];
    ...
    /* ---- Cleanup ---- */
    for (int i = 0; i < traveler_count; i++) {
        if (travelers[i].pipe_fd[0] != -1) close(travelers[i].pipe_fd[0]);
        if (travelers[i].ack_fd[1]  != -1) close(travelers[i].ack_fd[1]);
